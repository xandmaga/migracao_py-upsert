...
begin
  ...
  LOOP
    statement := 'COPY ' || ' (select row_to_json(' || tables.schema_table || ') from ' || tables.schema_table || ')' || ' TO ''' || path || '\\' || tables.schema_table || '.json''';
    EXECUTE statement;
  END LOOP;
  return;  
end;
...
begin
  ...
  LOOP
    ...
    statement := 'COPY ' || ' (select row_to_json(t) from (  select ' || columns_id || ' from ' || tables.schema_table || ' ) t )' || ' TO ''' || path || '\\' || tables.schema_table || '_ids' || '.json''';
    EXECUTE statement;
  END LOOP;
  return;  
end;
